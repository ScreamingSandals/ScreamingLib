...
public class SpongeMaterialMapping extends MaterialMapping {
    ...
    public SpongeMaterialMapping() {
        ...
        materialConverter
                ...
                .registerW2P(ItemStack.class, holder -> ItemStack.of(Sponge.getGame().registries().registry(RegistryTypes.ITEM_TYPE).findEntry(ResourceKey.resolve(holder.getPlatformName())).orElseThrow().value(), (byte) 1))
                ...
                .registerP2W(ItemStack.class, stack -> new MaterialHolder(Sponge.getGame().registries().registry(RegistryTypes.ITEM_TYPE).findValueKey(stack.getType()).orElseThrow().getFormatted()));
