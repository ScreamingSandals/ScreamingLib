...
public class ShadowColorResolver implements StylingResolver {
    @Override
    public <B extends Component.Builder<B, C>, C extends Component> void applyStyle(@NotNull MiniMessageParser parser, @NotNull B builder, @NotNull TagNode tag, @NotNull Placeholder @NotNull... placeholders) {
        ...
        var shadowColor = ShadowColor.hexOrName(tag.getArgs().get(0));

        if (tag.getArgs().size() > 1) {
            try {
                float alpha = Float.parseFloat(tag.getArgs().get(1));
                shadowColor = shadowColor.alpha((int) (alpha * 0xff));
