...
import org.screamingsandals.lib.player.event.PlayerJoinEvent;
...
public class PlayerJoinEventListener {
    ...
    public PlayerJoinEventListener() {
        MinecraftServer.getGlobalEventHandler().addEventCallback(PlayerLoginEvent.class, event ->
                EventManager.getDefaultEventManager().fireEvent(new PlayerJoinEvent(PlayerMapper.wrapPlayer(event.getPlayer()))));
    }
}
