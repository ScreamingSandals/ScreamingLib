...
import org.screamingsandals.lib.player.event.PlayerLeaveEvent;
...
public class PlayerLeaveEventListener {
    ...
    public PlayerLeaveEventListener() {
        MinecraftServer.getGlobalEventHandler().addEventCallback(PlayerDisconnectEvent.class, event ->
                EventManager.getDefaultEventManager().fireEvent(new PlayerLeaveEvent(PlayerMapper.wrapPlayer(event.getPlayer()))));
    }
