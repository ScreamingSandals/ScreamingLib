...
import org.screamingsandals.lib.bukkit.item.MaterialRegistry1_13_X;
...
public class BukkitBlockTypeRegistry1_13 extends BukkitBlockTypeRegistry {
    private static final @NotNull Map<@NotNull Material, List<String>> tagBackPorts = new HashMap<>();

    public BukkitBlockTypeRegistry1_13() {
        specialType(Material.class, mat -> mat.isBlock() ? new BukkitBlockType1_13(mat) : null);
        specialType(BlockData.class, BukkitBlockType1_13::new);
    ...
    protected @Nullable BlockType resolveMappingPlatform(@NotNull ResourceLocation location, @Nullable String blockState) {
        ...
        if (material != null && material.isBlock()) {
            if (blockState != null) {
                try {
                    return new BukkitBlockType1_13(material.createBlockData(blockState));
                } catch (IllegalArgumentException ignored) {
                    return null; // can't read the block state
                }
            } else {
                return new BukkitBlockType1_13(material);
            }
        }
        return null;
    }
