...
public class BukkitItemMergeEvent implements ItemMergeEvent, BukkitCancellable {
    @Getter
    @EqualsAndHashCode.Include
    @ToString.Include
    private final @NotNull org.bukkit.event.entity.ItemMergeEvent event;
    ...
    public @NotNull ItemEntity entity() {
        if (entity == null) {
            entity = new BukkitItemEntity(event.getEntity());
        }
        return entity;
    }

    @Override
    public @NotNull ItemEntity target() {
        if (target == null) {
            target = new BukkitItemEntity(event.getTarget());
        }
        return target;
    }
}
