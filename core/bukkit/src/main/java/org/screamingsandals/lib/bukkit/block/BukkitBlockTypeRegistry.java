...
@Service
public abstract class BukkitBlockTypeRegistry extends BlockTypeRegistry {
    @ServiceInitializer
    public static @NotNull BukkitBlockTypeRegistry init() {
        if (Version.isVersion(1, 14)) {
            return new BukkitBlockTypeRegistry1_14();
        } else if (Version.isVersion(1, 13)) {
            return new BukkitBlockTypeRegistry1_13();
        } else {
            return new BukkitBlockTypeRegistry1_8();
        }
    }
}
