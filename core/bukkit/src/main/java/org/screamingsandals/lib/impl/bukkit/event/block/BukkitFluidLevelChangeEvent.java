...
public class BukkitFluidLevelChangeEvent implements FluidLevelChangeEvent, BukkitCancellable {
    @Getter
    @EqualsAndHashCode.Include
    @ToString.Include
    private final @NotNull org.bukkit.event.block.FluidLevelChangeEvent event;

    // Internal cache
    private @Nullable BlockPlacement block;
    private @Nullable Block blockType;
    ...
    @Override
    public @NotNull Block newBlockData() {
        if (blockType == null) {
            blockType = Block.of(event.getNewData());
        }
        return blockType;
    }
}
