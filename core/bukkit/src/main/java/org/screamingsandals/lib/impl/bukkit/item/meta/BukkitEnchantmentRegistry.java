...
@Service
public class BukkitEnchantmentRegistry extends EnchantmentRegistry {

    public BukkitEnchantmentRegistry() {
        specialType(org.bukkit.enchantments.Enchantment.class, BukkitEnchantment::new);
    }
    ...
    protected @Nullable Enchantment resolveMappingPlatform(@NotNull ResourceLocation location) {
        if (BukkitFeature.REGISTRY.isSupported()) {
            ...
            if (value != null) {
                return new BukkitEnchantment(value);
            }
        }
        return null;
    }
    ...
    protected @NotNull RegistryItemStream<@NotNull Enchantment> getRegistryItemStream0() {
        if (BukkitFeature.FLATTENING_MATERIAL.isSupported()) {
            ...
            return new SimpleRegistryItemStream<>(
                    () -> Arrays.stream(org.bukkit.enchantments.Enchantment.values()),
                    BukkitEnchantment::new,
                    enchantment -> ResourceLocation.of(BukkitEnchantment.getLocationPath(enchantment)),
                    (enchantment, literal) -> BukkitEnchantment.getLocationPath(enchantment).contains(literal),
