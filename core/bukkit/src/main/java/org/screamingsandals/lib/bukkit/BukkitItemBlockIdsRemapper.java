...
import org.screamingsandals.lib.bukkit.block.BlockDataTranslatorBuilder;
...
public class BukkitItemBlockIdsRemapper extends ItemBlockIdsRemapper {
    ...
    public void doMapping() {
        if (bPlatform == Platform.JAVA_LEGACY) {
    ...
    private void flatteningLegacyMappingJava() {
        ...
        var gate = new BlockDataTranslatorBuilder()
                ...
                .maskedBoolean("open", 0x4)
                .build();
        blockTypeMapper.getBlockDataTranslators().put(k -> k.path().toUpperCase(Locale.ROOT).endsWith("FENCE_GATE"), gate);
