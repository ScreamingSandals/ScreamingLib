...
import org.screamingsandals.lib.bukkit.block.BukkitBlockTypeRegistry1_8;
import org.screamingsandals.lib.bukkit.item.BukkitItemType1_8;
import org.screamingsandals.lib.bukkit.item.BukkitItemTypeRegistry1_8;
...
public class BukkitItemBlockIdsRemapper extends ItemBlockIdsRemapper {
    public BukkitItemBlockIdsRemapper(@NotNull ItemTypeRegistry itemTypeMapper, @NotNull BlockTypeRegistry blockTypeMapper) {
    ...
    protected void f2lBlock(@NotNull String flatteningMaterial, @NotNull String legacyBukkitMaterial, int data) {
        ...
        if (!(blockTypeMapper instanceof BukkitBlockTypeRegistry1_8)) {
            return;
        }
        ...
        try {
            ...
            var holder = new BukkitBlockType1_8(mat, (byte) data);

            var mapping = ((BukkitBlockTypeRegistry1_8) blockTypeMapper).getPorts();
            if (!mapping.containsKey(flatteningMaterialNamespaced)) {
                mapping.put(flatteningMaterialNamespaced, holder);
            }

            var locations = ((BukkitBlockTypeRegistry1_8) blockTypeMapper).getResourceLocations();
            if (!locations.containsKey(holder)) {
                locations.put(holder, flatteningMaterialNamespaced);
            }
        } catch (IllegalArgumentException ignored) {
