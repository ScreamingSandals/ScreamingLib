...
public class BukkitBlockExplodeEvent implements BlockExplodeEvent, BukkitCancellable {
    @Getter
    @EqualsAndHashCode.Include
    @ToString.Include
    private final @NotNull org.bukkit.event.block.BlockExplodeEvent event;

    // Internal cache
    private @Nullable BlockPlacement block;
    private @Nullable Collection<@NotNull BlockPlacement> destroyed;
    ...
    @Override
    public @NotNull Collection<@NotNull BlockPlacement> destroyedBlocks() {
        if (destroyed == null) {
            destroyed = new CollectionLinkedToCollection<>(event.blockList(), o -> o.as(org.bukkit.block.Block.class), BukkitBlockPlacement::new);
        }
        return destroyed;
    }
